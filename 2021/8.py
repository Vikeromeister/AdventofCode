input = """ceg gedcfb ec eabfdg gcdabe baged cabgf gbaec fecagdb eacd | efcgbad adfecbg gec abgce
...
aed aecfbdg geafc cdeaf efdcb bcgedf bdaf da gbdcae beacfd | ecafd ade cdfae bedcf""".splitlines()

uniques = []
outputs = []
for line in input:
    tempunique = line.split(" | ")[0]
    uniques.append([])
    for num in tempunique.split(" "):
        uniques[-1].append((num))
    tempoutput = line.split(" | ")[1]
    outputs.append([])
    for num in tempoutput.split(" "):
        outputs[-1].append(num)
...
def getletters(words):
    ...
    words.sort(key=len)
    if words[1][0] not in words[0]:
        a = words[1][0]
    elif words[1][1] not in words[0]:
        a = words[1][1]
    elif words[1][2] not in words[0]:
        a = words[1][2]
    else:
        print("ERRROR: a not found")
...
def getnums(word, a, b, c, d, e, f, g):
    if len(word) == 2:
        return 1
    if len(word) == 3:
        return 7
    ...
    if len(word) == 5:
        ...
        if c not in word:
            return 5
        return 3
    print("ERROR: no number is recognized")


sum = 0
for i in range(len(outputs)):
    [a, b, c, d, e, f, g] = getletters(uniques[i])
    number = getnums(outputs[i][0], a, b, c, d, e, f, g) * 1000 \
             + getnums(outputs[i][1], a, b, c, d, e, f, g) * 100 \
             + getnums(outputs[i][2], a, b, c, d, e, f, g) * 10 \
             + getnums(outputs[i][3], a, b, c, d, e, f, g)
    print(number)
    sum += number
print(sum)
