input = """...733.......289..262.....520..................161.462..........450.........................183.............................................
...
def thisnumber(x, y):
    global numbers
    while y >= 0 and input[x][y] in numbers:
        y -= 1
    y+=1
    start = y
    num = ''
    try:
        while input[x][y] in numbers:
            num += input[x][y]
            y += 1
    except:
        pass
...
doneThisNumber = False
total = 0
totalratio = 0
for i, line in enumerate(input):
    doneThisNumber = False
    for j, char in enumerate(line):
        if char in numbers and not doneThisNumber:
            doneThisNumber = True
            nrStart = j
            k = j
            number = ''
            while k < len(line) and line[k] in numbers:
                nrStop = k
                number += line[k]
                k += 1
            valid = False
            if i > 0:    
                for check in input[i-1][nrStart:nrStop+1]:
                    if check not in numbers and check != '.':
                        valid = True
                if nrStart > 0:
                    if input[i-1][nrStart-1] not in numbers and input[i-1][nrStart-1] != '.':
                        valid = True
                if nrStop < len(line) - 1:
                    if input[i-1][nrStop+1] not in numbers and input[i-1][nrStop+1] != '.':
                        valid = True
            if i < len(input)-1:    
                for check in input[i+1][nrStart:nrStop+1]:
                    if check not in numbers and check != '.':
                ...
                if nrStop < len(line) - 1:
                    if input[i+1][nrStop+1] not in numbers and input[i+1][nrStop+1] != '.':
                        valid = True
            if nrStart > 0:
                if line[nrStart-1] not in numbers and line[nrStart-1] != '.':
                    valid = True
            if nrStop < len(line)-1:
                if line[nrStop+1] not in numbers and line[nrStop+1] != '.':
                    valid = True
            if valid:
                # print(number)
                total += int(number)
        ...
        if char == '*':
            ...
            if i > 0:
                if j > 0:
                    if input[i-1][j-1] in numbers:
                        count += 1
                        power = power * thisnumber(i-1, j-1)[0]
                        start = thisnumber(i-1, j-1)[1]
                if input[i-1][j] in numbers:
                    if start != thisnumber(i-1, j)[1]:
                        count += 1
                        power = power * thisnumber(i-1, j)[0]
                        start = thisnumber(i-1, j)[1]
                if j < len(line) - 1:
                    if input[i-1][j+1] in numbers:
                        if start != thisnumber(i-1, j+1)[1]:
                            count += 1
                            power = power * thisnumber(i-1, j+1)[0]

            if j > 0:
                if input[i][j-1] in numbers:
                    count += 1
                    power = power * thisnumber(i, j-1)[0]
            if j < len(line) - 1:
                if input[i][j+1] in numbers:
                    count += 1
                    power = power * thisnumber(i, j+1)[0]
