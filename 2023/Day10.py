input = """F|--L7J.L7|7|-LF7-7--7F7--7F7J--F7J.7F|J7.F7|.-FJF|J.F7F--LFF.7.F7JFL-JFJ7F|.FL77-F7-J.F7-F7L-FF-F-L7F-|7.FL7-7F|7F7.F-F|-77FJ7FF-J777.|FFFL
...
77LL-J.-JJ-F--L-J-LF-F--7-.--|--LJ-FL-JJL7.7-JJ.FLJ-7-JLLL--7-|-|LLL7.--J.JJ-LF7-LLJ-L|J-LJ.LL7.LFJ.L|L-J--LL|7-7LJ-J|-7-J.|JL|JJ.7-L|JLJL.L""".splitlines()
...
while not done:
    try:
        location[0] += move[next][0]
        location[1] += move[next][1]
        length += 1
        if location[0] == start[0] and location[1] == start[1]:
            print(length, length/2)
            done = True
        next = lookup[input[location[0]][location[1]]][next]
        if -1 in location:
            break
